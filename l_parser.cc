...
namespace
{
	...
	void print_system(std::ostream&out, LParser::LSystem const& system)
	{
		...
		else if (system.get_alphabet().size() == 1)
		{
            ...
            for (int i = 0; i < system.get_replacement(*system.get_alphabet().begin()).size(); ++i) {
                out << "Rules = {" << *system.get_alphabet().begin() << " -> \"" << system.get_replacement(*system.get_alphabet().begin())[i].first << "\"}" << std::endl;
            }
		...
		else
		{
            ...
            for (int i = 0; i < system.get_replacement(*system.get_alphabet().begin()).size(); ++i) {
                out << "Rules = {" << *system.get_alphabet().begin() << " -> \"" << system.get_replacement(*system.get_alphabet().begin())[i].first << "\"}";
            }

			for (std::set<char>::const_iterator i = ++system.get_alphabet().begin(); i != system.get_alphabet().end(); i++)
			{
                for (int j = 0; j < system.get_replacement(*system.get_alphabet().begin()).size(); ++j) {

                    out << "," << std::endl << "\t" << *i << " -> \"" << system.get_replacement(*i)[j].first << "\"";
                }
...
//                throw LParser::ParserException(std::string("Double entry '") + c + "' in rules specification ", parser.getLine(), parser.getCol());
            ...
            //48 is ascii waarde van 0
            if (parser.peekChar() == 48){
                percentage = parser.readDouble();
            }
			...
			rules[alphabet_char].push_back(make_pair(rule, percentage));
			parser.skip_comments_and_whitespace();
			c = parser.getChar();
			if (c == '}')
				break;
...
std::vector<pair<std::string, double>> const& LParser::LSystem::get_replacement(char c) const
{
	assert(get_alphabet().find(c) != get_alphabet().end());

	//nemen een random waarde tussen te grote van de vector
//	int random = rand() % replacementrules.find(c)->second.size();
